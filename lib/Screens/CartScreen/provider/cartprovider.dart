    ...
    try {
      ...
      if (response.statusCode == 200) {
       ...
       _carts = [];
        ...
        final List<dynamic> cartDetails = extractedData['cart_items'];
        for (var i = 0; i < cartDetails.length; i++) {
          _carts.add(
            CartModel(
                id: cartDetails[i]['id'].toString(),
                productId: cartDetails[i]['product_id'].toString(),
                quantity: cartDetails[i]['quantity'].toString(),
                userId: cartDetails[i]['user_id'].toString(),
                checkoutStatus: cartDetails[i]['checkout_status'].toString(),
                createdAt: cartDetails[i]['created_at'].toString(),
                productName: cartDetails[i]['product_name'].toString(),
                categoryId: cartDetails[i]['category_id'].toString(),
                file: cartDetails[i]['file'].toString(),
                productQuantity: cartDetails[i]['product_quantity'].toString(),
                description: cartDetails[i]['description'].toString(),
...
Future<void> addItemToCart(
  ...
  Future<void> clearCart({String? userid}) async {
    final url = Uri.parse(
        'http://campus.sicsglobal.co.in/Project/homemade_crafts/API/empty_cart.php?userid=$userid');
